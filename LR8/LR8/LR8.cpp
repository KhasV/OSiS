	...
	case WM_DRAWITEM:
	{
		...
		case BUTTON_DRAW_ID:
		{
			...
			if (draw->itemAction == ODA_SELECT) {
				brush = CreateSolidBrush(RGB(224, 151, 94));
			}
			else {
				brush = CreateSolidBrush(RGB(95, 184, 138));
			}
		...
		case BUTTON_CLEAR_ID:
		{
			...
			if (draw->itemAction == ODA_SELECT) {
				...
				brush = CreateSolidBrush(RGB(94, 122, 224));
			}
			pen = CreatePen(BS_SOLID, 2, RGB(0, 0, 0));
    ...
    case WM_PAINT:
        {
			...
			if (isVisible)
			{
				HBRUSH brush = CreateSolidBrush(RGB(0, 50, 50));
				SelectObject(hdc, brush);
				...
				brush = CreateSolidBrush(RGB(56, 75, 199));
				SelectObject(hdc, brush);
